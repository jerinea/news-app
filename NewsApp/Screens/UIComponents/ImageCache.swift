...
private let ImageCacheClearCacheAfterHours = 1
private let _ImageCacheSharedInstance = ImageCache()

class ImageCache: NSObject, URLSessionDelegate {
    ...
    fileprivate func loadFromUrl(_ imageUrl: String, size: CGSize, completion: @escaping (_ image: UIImage?) -> Void) {
        ...
        let task = session.dataTask(with: request, completionHandler: {
            ...
            if data != nil {
                ...
                if image == nil {
                    print("Image Found With Error: \(imageUrl)")
                }
                
                // To resize the image if needed
    ...
    fileprivate func saveInFileSystem(_ image: UIImage?, imageUrl: String, size: CGSize) {
        ...
        if image != nil {
            ...
            if(imageUrl.lowercased().range(of: ".png") != nil) {
                try? image!.pngData()!.write(to: URL(fileURLWithPath: uniquePath), options: [.atomic])
            } else if(imageUrl.lowercased().range(of: ".jpg") != nil || imageUrl.lowercased().range(of: ".jpeg") != nil) {
                try? image!.jpegData(compressionQuality: 100)!.write(to: URL(fileURLWithPath: uniquePath), options: [.atomic])
            }
        }
    }
